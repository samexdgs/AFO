"""
app.py  —  AFO Clinical Management Platform v3
All visuals now use the uploaded real assets:
  - Lottie JSON files loaded from local assets/ folder (base64 encoded inline)
  - hpls_afo.png AFO comparison diagram
  - Education: matched Lottie to each topic
  - Recommendations: Ankle_Brace + TENS Lottie per AFO type
...
def _load_lottie(filename: str) -> str:
    """Return base64-encoded Lottie JSON string, or empty string on failure."""
...
def _lottie_html(b64: str, height: int = 200) -> str:
    """Render a base64 Lottie JSON as a self-contained HTML snippet."""
    if not b64:
        return "<p style='color:#94a3b8;font-size:0.8rem'>Animation unavailable</p>"
    return f"""
    <html><head>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/bodymovin/5.12.2/lottie.min.js"></script>
    <style>body{{margin:0;background:transparent;display:flex;
      align-items:center;justify-content:center;height:{height}px}}
    #a{{width:{height}px;height:{height}px}}</style>
    </head><body><div id="a"></div>
    <script>
    const data = JSON.parse(atob('{b64}'));
    lottie.loadAnimation({{container:document.getElementById('a'),
      renderer:'svg',loop:true,autoplay:true,animationData:data}});
    </script></body></html>"""


# Pre-load all assets once at startup
_LOTTIE = {
    "ankle1":    _load_lottie("Ankle_Brace__1_.json"),
    "ankle2":    _load_lottie("Ankle_Brace__2_.json"),
    "tens":      _load_lottie("TENS_devices.json"),
    "walk":      _load_lottie("Man_Walk.json"),
    "treadmill": _load_lottie("Man_Walking_On_Treadmill.json"),
    "calf":      _load_lottie("Man_Doing_Calf_Raise_Exercise_for_Legs.json"),
    "rolling":   _load_lottie("Man_Rolling_Calf_Exercise_For_Leg_Relaxed.json"),
    "side_leg":  _load_lottie("Side_Right_Leg_Raises_Exercise.json"),
    "calf_w":    _load_lottie("Woman_Doing_Dumbbell_Standing_Single_Leg_Deficit_Calf_Raise_Exercise_for_Calves.json"),
    "band":      _load_lottie("Woman_Doing_Resistance_Band_Standing_Leg_Curl_Exercise_for_Legs.json"),
    "plank":     _load_lottie("Leg_Pull_Low_Plank.json"),
    "lunges":    _load_lottie("Plank_Lunges_Exercise.json"),
}

_AFO_DIAGRAM_B64 = _load_img_b64("hpls_afo.png")

# Which Lottie to show for each AFO type key
_AFO_LOTTIE = {
    "pls":    "ankle1",   # Posterior Leaf Spring — ankle brace 1
    "solid":  "ankle2",   # Solid ankle — ankle brace 2 (more detail)
    "hinged": "ankle2",   # Hinged — ankle brace 2
    "carbon": "ankle1",   # Carbon fibre — ankle brace 1 (slim profile)
    "grafo":  "band",     # GRAFO — resistance band leg exercise (crouch gait context)
    "fes":    "tens",     # FES — TENS/electrical stimulation device (perfect match)
}
...
def _alert(text: str, kind: str = "info"):
    c = {"info":  ("#eff6ff","#bfdbfe","#1e40af"),
         "warn":  ("#fefce8","#fef08a","#854d0e"),
         "error": ("#fff1f2","#fecaca","#991b1b")}
    bg, border, fg = c.get(kind, c["info"])
    st.markdown(
...
def _wearing_chart(logs: list) -> go.Figure | None:
    ...
    fig = go.Figure(go.Bar(
        x=pd.to_datetime(df["date"]), y=df["worn"].astype(int),
...
def _model_perf_chart(models: dict) -> go.Figure:
    ...
    fig.add_trace(go.Bar(name="Accuracy", x=names,
        y=[models[n]["accuracy"] for n in names], marker_color="#2563eb",
        text=[f"{models[n]['accuracy']:.1%}" for n in names], textposition="outside"))
    fig.add_trace(go.Bar(name="ROC-AUC", x=names,
        y=[models[n]["roc_auc"] for n in names], marker_color="#059669",
        text=[f"{models[n]['roc_auc']:.3f}" for n in names], textposition="outside"))
    fig.update_layout(barmode="group", height=240, margin=dict(l=10,r=10,t=10,b=10),
...
# ── Recommendation renderer with real Lottie assets ───────────────────────────
...
def _render_recommendation(label: int, proba: list, assessment: dict,
    ...
    for rank, idx in enumerate(top3):
        ...
        if key == "fes" and assessment.get("has_pacemaker"):
            contra_warning = ('<div style="background:#fff1f2;border:1px solid #fecaca;'
            ...
            contra_warning = ('<div style="background:#fefce8;border:1px solid #fde68a;'
                              'border-radius:6px;padding:6px 10px;font-size:0.82rem;'
                              'color:#854d0e;margin:8px 0">⚠ Carbon fibre insufficient for MAS ≥ 3</div>')

        # Build the text detail block
        if audience == "physio":
            ind_rows = "".join(f'<div style="font-size:0.82rem;color:#166534;margin-bottom:3px">✓ {i}</div>'
                               for i in afo["indicated_when"][:3])
            con_rows = "".join(f'<div style="font-size:0.82rem;color:#991b1b;margin-bottom:3px">✗ {c}</div>'
                               for c in afo["contraindicated_when"][:2])
            phases   = " · ".join(afo["gait_phases"])
            text_block = f"""
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:10px">
              <div><div style="font-size:0.72rem;font-weight:600;color:#166534;
                   text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">Indicated when</div>
                {ind_rows}</div>
              <div><div style="font-size:0.72rem;font-weight:600;color:#991b1b;
                   text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">Avoid when</div>
                {con_rows}</div>
            </div>
        ...
        <div style="border:{border};border-radius:12px;padding:1.1rem 1.3rem;
                    margin-bottom:0.5rem;background:{bg};font-family:'IBM Plex Sans',sans-serif">
          <div style="font-size:0.7rem;font-weight:600;color:#64748b;
                      text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">{rank_label}</div>
        ...
        # Show card + Lottie side by side
        col_text, col_anim = st.columns([2.2, 1])
        with col_text:
            components.html(card_html,
                            height=300 if audience == "physio" else 230,
                            scrolling=False)
        with col_anim:
            lottie_key = _AFO_LOTTIE.get(key, "ankle1")
            b64 = _LOTTIE.get(lottie_key, "")
            components.html(_lottie_html(b64, height=180), height=190, scrolling=False)

            # For rank 0 only, also show the AFO diagram image
            if rank == 0 and _AFO_DIAGRAM_B64:
                st.markdown(
                    f'<img src="data:image/png;base64,{_AFO_DIAGRAM_B64}" '
                    f'style="width:100%;border-radius:8px;border:1px solid #e2e8f0;'
                    f'margin-top:4px" alt="AFO types diagram"/>',
                    unsafe_allow_html=True)
                st.caption("AFO types: SAFO · DAFO · HAFO · GFAFO · PLS AFO")
...
def _page_login():
    ...
    with col:
        st.markdown('<p style="font-size:1.7rem;font-weight:600;color:#0f172a;'
                    'letter-spacing:-0.02em">🦿 AFO Clinical Platform</p>',
                    unsafe_allow_html=True)
        st.markdown('<p style="font-size:0.88rem;color:#64748b;margin-bottom:1rem">'
                    'Evidence-based ankle-foot orthosis management</p>',
                    unsafe_allow_html=True)
        st.markdown("---")
        ...
        st.markdown('<div style="background:#f1f5f9;border:1px solid #cbd5e1;'
                    'border-radius:8px;padding:0.7rem 0.9rem;font-size:0.76rem;'
                    'color:#475569;margin-top:1rem">Demo platform. ML recommendations only. '
                    'Built by Samuel Oluwakoya.</div>', unsafe_allow_html=True)
...
def _physio_portal(models: dict, best: str):
    ...
    if nav == "Patient Panel":       _physio_patient_panel(patients, models, best)
    elif nav == "New Assessment":    _physio_assessment_form(patients, models, best)
    elif nav == "Register Patient":  _physio_register_patient(physio["username"])
    elif nav == "Cohort Overview":   _physio_cohort(patients)
    elif nav == "Model Performance": _physio_model_perf(models, best)


def _physio_patient_panel(patients, models, best):
    ...
    if assessment:
        ...
        with tab_rec:
            lbl, proba = clinical.predict(models, best,
                                          {k:assessment.get(k,0) for k in FEATURES})
        ...
        with tab_sat:
            sat = store.satisfaction_get(patient["patient_id"])
            if not sat:
                st.info("No satisfaction assessments submitted yet.")
            else:
                latest = sat[-1]
                total  = round(sum(latest["scores"].values())/len(latest["scores"]),2)
                st.markdown(f"**Latest score: {total}/5** ({latest['date']})")
                if total < 3.0:
                    _alert("⚠ Score below 3.0 — consider scheduling a fitting review.","warn")
                ...
                for entry in reversed(sat):
                    avg = round(sum(entry["scores"].values())/len(entry["scores"]),2)
...
def _physio_assessment_form(patients, models, best):
    ...
    if submitted:
        ...
        row = {"age":patient["age"],"days_post_onset":days_post,
...
def _physio_register_patient(physio_username):
...
def _physio_cohort(patients):
    ...
    for p in patients:
        ...
        rows.append({"Name":p["full_name"],"ID":p["patient_id"],"Age":p["age"],
                     "Diagnosis":p["diagnosis"],"Days post-onset":p["days_post_onset"],
    ...
    with c2: _stat("Assessed", sum(1 for p in patients
                                   if store.assessment_latest(p["patient_id"])))
    with c3: _stat("Logging wearing", sum(1 for p in patients
                                          if store.wearing_log_get(p["patient_id"])))


def _physio_model_perf(models, best):
    _title("Model Performance")
    st.markdown(f"Active model: **{best}**")
    st.plotly_chart(_model_perf_chart(models), use_container_width=True)
    st.markdown("""
**Training:** 4,000 synthetic records · Feature schema: Lee et al. (2021) PMC8055674
**Classes:** PLS · Solid Ankle · Hinged · Carbon Fibre · GRAFO · FES
""")


# ── Prescription export ───────────────────────────────────────────────────────

def _prescription_export(patient, assessment, label, proba, stiff):
    ...
    questions = [
        ...
        f"Stiffness score <strong>{stiff}/10</strong> — recommended spring constant?",
        "Custom-moulded or off-the-shelf?",
        "What shoe modifications are needed?",
        "Is FES a viable alternative at this stage?",
    ...
    with st.expander("🖨 View prescription summary for orthotist"):
        ...
        txt = (f"AFO PRESCRIPTION — {today}\nPatient: {patient['full_name']} | "
               ...
               f"RECOMMENDATION: {top_afo['name']} | Confidence: {proba[label]:.0%} | "
               f"Stiffness: {stiff}/10\nCost: {top_afo['cost_usd']} / {top_afo['cost_ngn']}\n"
               f"Samuel Oluwakoya — AFO Clinical Platform\n")
        st.download_button("⬇ Download (.txt)", txt,
                           f"rx_{patient['patient_id']}_{date.today()}.txt", "text/plain")


# ── Patient portal ────────────────────────────────────────────────────────────

def _patient_portal(models, best):
    ...
    with st.sidebar:
        ...
        if st.button("Sign out"):
            _logout()
    if nav == "My AFO":       _patient_my_afo(patient, models, best)
    elif nav == "Daily Log":  _patient_wearing_log(patient)
    elif nav == "Satisfaction": _patient_satisfaction(patient)
    elif nav == "Education":  _patient_education()


def _patient_my_afo(patient, models, best):
    _title("My AFO Recommendation")
    assessment = store.assessment_latest(patient["patient_id"])
    if not assessment:
        st.info("Your physiotherapist has not completed a clinical assessment yet.")
        return
    label, proba = clinical.predict(models, best,
                                    {k:assessment.get(k,0) for k in FEATURES})
    top_key = LABEL_TO_KEY[label]
    top_afo = AFO_TYPES[top_key]
    for a in clinical.reassessment_alerts(
    ...
    <div style="border:2px solid #2563eb;border-radius:12px;padding:1.2rem 1.4rem;
      ...
      <div style="margin-top:8px;font-size:0.8rem;color:#64748b">
        Cost: {top_afo['cost_usd']} / {top_afo['cost_ngn']}</div>
    </div>""", height=195, scrolling=False)
    ...
    for e in reversed(sorted(wearing, key=lambda x: x["date"])):
        if e.get("worn"): streak += 1
        else: break
    if total_d:
        c1,c2,c3 = st.columns(3)
        with c1: _stat("Days worn", worn_days)
        with c2: _stat("Streak 🔥", streak)
...
def _patient_wearing_log(patient):
    ...
    with col_form:
        if today_entry:
            st.success(f"Today logged: {'✅ Worn' if today_entry['worn'] else '❌ Not worn'} "
                       f"· {today_entry.get('hours',0)} hours")
        else:
            ...
            note  = st.text_input("Notes?", placeholder="e.g. rubbing at heel")
            if st.button("Save today's entry"):
                store.wearing_log_save(patient["patient_id"],
                    {"worn":worn=="Yes","hours":hours,"comfort":pain,"note":note})
                st.rerun()
    with col_chart:
        if len(log)>=2:
    ...
    if log:
        ...
        if worn_n/len(log)<0.7:
            _alert("⚠ Compliance below 70%. If discomfort is the reason, "
...
def _patient_satisfaction(patient):
    ...
    with st.form("satisfaction"):
        ...
        if st.form_submit_button("Save assessment"):
            ...
            st.success("Scores saved. Your physiotherapist can now see them.")
    ...
    if history:
        ...
        st.plotly_chart(fig, use_container_width=True)
        if total < 3.0:
            _alert("Score is low. Mention this at your next appointment — "
                   "fit and comfort issues are usually fixable.","warn")


# ── Education with real Lottie assets ─────────────────────────────────────────

_EDU = [
    {
        "q": "What is foot drop and why do I need an AFO?",
        "a": ("Foot drop is the inability to lift the front part of your foot because the muscles "
              "controlled by the peroneal nerve are weak after stroke. When you walk, the foot drags "
              "or catches, causing falls. An AFO holds your foot at the correct angle so you can walk "
              "safely while your nervous system continues to recover."),
        "lottie": "walk",
        "link": "https://www.stroke.org.uk/effects-of-stroke/physical-effects/foot-drop",
        "link_label": "Stroke Association: Foot Drop Guide",
    },
    {
        "q": "Will my AFO cure foot drop?",
        "a": ("An AFO is a functional support tool, not a cure. It improves how you walk and reduces "
              "fall risk while your nervous system continues to recover. Many patients with early "
              "post-stroke foot drop see real improvements over 6 months. Your physiotherapist will "
              "monitor whether you still need the AFO at each review."),
        "lottie": "treadmill",
    ...
    },
    {
        ...
        "a": ("In early recovery, wear your AFO throughout all walking activity — typically 6–12 hours. "
              "As strength returns your physiotherapist may trial periods without it. Never stop "
              "wearing it without clinical guidance. Falls are the main risk when it is removed too early."),
        "lottie": "rolling",
    ...
    {
        "q": "My AFO is causing pain — what do I do?",
        "a": ("Stop wearing it and check your skin immediately. Redness lasting more than 20 minutes "
              "after removal, or blistering, means a pressure point that must be fixed by your orthotist "
              "before you wear it again. Do not push through pain. A single adjustment solves most problems."),
        "lottie": "calf",
        "link": "https://www.orthoinfo.aaos.org/en/treatment/ankle-foot-orthosis-afo/",
        "link_label": "AAOS: AFO Care and Fitting Guide",
    },
    {
        "q": "What shoes can I wear with my AFO?",
        "a": ("You typically need one full shoe size larger on the affected foot. Wide toe-box lace-up "
              "trainers or sports shoes work best. Slip-ons and heels are generally incompatible with "
              "solid and hinged designs. Your orthotist can advise on specific brands."),
        "lottie": "calf_w",
        "link": "https://www.feetforlife.org",
        "link_label": "Feet for Life: AFO-Compatible Footwear",
    },
    {
        "q": "What is FES and how is it different from an AFO?",
        "a": ("FES (Functional Electrical Stimulation) uses electrode pads on your lower leg that "
              "send electrical pulses to activate the muscles that lift your foot. Unlike a passive "
              "AFO brace, FES actively fires the muscle and may help re-train the nervous system "
              "over time. It is most effective in the first 6 months post-stroke."),
        "lottie": "tens",
        "link": "https://www.bioness.com/patients/l300-plus.php",
        "link_label": "Bioness L300: FES for Foot Drop",
    },
    {
        "q": "When will I be reassessed?",
        "a": ("The recommended schedule is every 4–6 weeks in the first 6 months, then every 3–6 months. "
              "Motor recovery is fastest in the first 3 months — this is when reassessments matter most. "
              "Request an early appointment if: the AFO no longer fits, walking has changed significantly, "
              "or you have skin problems."),
        "lottie": "side_leg",
        "link": "https://www.physiotherapy.asn.au",
        "link_label": "APA: What to Expect at Your Review",
    },
]


def _education_card(item: dict, idx: int):
    b64   = _LOTTIE.get(item["lottie"], "")
    html  = _lottie_html(b64, height=170)
    with st.expander(item["q"], expanded=(idx == 0)):
        ...
        with col_text:
            st.markdown(
                ...
                f'margin-top:0.4rem">{item["a"]}</p>', unsafe_allow_html=True)
            st.markdown(
                f'<a href="{item["link"]}" target="_blank" '
                f'style="display:inline-block;background:#2563eb;color:white;'
                f'padding:7px 14px;border-radius:8px;font-size:0.82rem;'
                f'font-weight:500;text-decoration:none;margin-top:8px">'
                f'🔗 {item["link_label"]}</a>', unsafe_allow_html=True)

        # Show the AFO diagram for the first two cards
        if idx < 2 and _AFO_DIAGRAM_B64:
            st.image(f"data:image/png;base64,{_AFO_DIAGRAM_B64}",
                     caption="AFO types: SAFO · DAFO · HAFO · GFAFO · PLS AFO",
                     use_container_width=True)


def _patient_education():
    _title("Understanding Your AFO")
    st.markdown("Everything you need to know — with animations and trusted links.")
    st.markdown("")
    for i, item in enumerate(_EDU):
